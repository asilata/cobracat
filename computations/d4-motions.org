...
#+begin_src sage :results none
  V = var('X1', 'X2', 'X3', 'Y1', 'Y2', 'Y3', 'Z1', 'Z2', 'Z3', 'A', 'B', 'C')
  Xs = {1:X1, 2:X2, 3:X3}
  Ys = {1:Y1, 2:Y2, 3:Y3}
  Zs = {1:Z1, 2:Z2, 3:Z3}
#+end_src
...
#+begin_src sage
  def simplicies(i):
      return [x for x in Set(V).subsets(i) if any([set(x).issubset(set(s)) for s in states])]
...
#+begin_src sage
  forbidden2 = [x for x in Set(V).subsets(2) if Set(x) not in simplicies(2)]
  [y for y in [x for x in Set(V).subsets(3) if all([not f.issubset(x) for f in forbidden2])] if y not in simplicies(3)]
#+end_src
...
#+begin_src sage
  forbidden2 = [x for x in Set(V).subsets(2) if Set(x) not in simplicies(2)]
  forbidden3 = [x for x in Set(V).subsets(3) if Set(x) not in simplicies(3)]
  [y for y in [x for x in Set(V).subsets(4) if all([not f.issubset(x) for f in forbidden2 + forbidden3])] if y not in simplicies(4)]
#+end_src
...
#+begin_src sage
  forbidden2 = [x for x in Set(V).subsets(2) if Set(x) not in simplicies(2)]
  forbidden3 = [x for x in Set(V).subsets(3) if Set(x) not in simplicies(3)]
  forbidden4 = [x for x in Set(V).subsets(4) if Set(x) not in simplicies(4)]
  len([y for y in [x for x in Set(V).subsets(5) if all([not f.issubset(x) for f in forbidden2 + forbidden3 + forbidden4])]])
#+end_src
...
#+begin_src  sage :results file
  central_charge_plot = sum([point(central_change_euclidean.rows()[i], color='blue') for i in range(0,4)]) + sum([point(-1*central_change_euclidean.rows()[i], color='red') for i in range(0,4)]) + point((0,0), color='black')
#+end_src

#+RESULTS:
[[file:/home/anandrd/.sage/temp/proteus/402480/tmp_d18myl12.png]]
...
#+begin_src  sage
  ...
  #+begin_src sage :results file
    ...
    def plot_object(x, color='blue'):
            xclass = classes[x]
            return sum([arrow(t * vector(ambient_basis[j]) * central_change_euclidean, s * vector(ambient_basis[i]) * central_change_euclidean, linestyle='dashed', color=color) for ((s,i), (t,j)) in roots_as_differences[xclass]])
...
#+RESULTS:
[[file:/home/anandrd/.sage/temp/proteus/402480/tmp_dzcq_zt7.png]]
