...
#+RESULTS:
[[file:/home/anandrd/.sage/temp/proteus/402480/tmp_gv55wh6_.png]]
...
#+begin_src  sage
  ...
  #+begin_src sage :results file
    ...
    def plot_object(x, color='blue'):
            xclass = classes[x]
            return sum([arrow(t * vector(ambient_basis[j]) * central_charge_euclidean, s * vector(ambient_basis[i]) * central_charge_euclidean, color=color, width=1,arrowsize=2) for ((s,i), (t,j)) in roots_as_differences[xclass]])
...
#+RESULTS:
[[file:/home/anandrd/.sage/temp/proteus/402480/tmp_jodegujj.png]]
...
#+begin_src sage :results file
  ...
  (plot_state(states[i]) + plot_motion(V) + central_charge_plot).show(aspect_ratio=1, axes=False)
#+end_src

#+RESULTS:
[[file:/home/anandrd/.sage/temp/proteus/402480/tmp_nzwklppu.png]]

#+begin_src sage
  [classes[x] for x in states[11]]
#+end_src
